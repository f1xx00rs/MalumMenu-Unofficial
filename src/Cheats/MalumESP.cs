...
public static class MalumESP
{
        ...
        if (CheatToggles.zoomOut)
        {
            ...
            else if (Input.GetAxis("Mouse ScrollWheel") > 0f)
            {
                ...
                if (!(Camera.main.orthographicSize > 3f))
                    return; // Never go below the default orthographicSize: 3f
    ...
    public static void ChatNametags(ChatBubble chatBubble)
    {
        try
        {
            ...
            // ВАЖНО:
            // Используем чистое имя игрока, а не chatBubble.NameText.text
            // Иначе точки/теги начинают дублироваться каждый апдейт

            chatBubble.NameText.text =
                dot + " " +
                Utils.GetNameTag(
                    chatBubble.playerInfo,
                    chatBubble.playerInfo.PlayerName,
                    true
                );
    ...
    public static void SeeGhostsCheat(PlayerPhysics playerPhysics)
    {
        ...
        {
            if (
                playerPhysics.myPlayer.Data.IsDead &&
